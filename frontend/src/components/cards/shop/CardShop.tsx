...
const CardShop: FC<CardShopProps> = ({ isFor, platform }) => {
  return (
    <CardContain
      ...
      urlImage={`https://via.placeholder.com/${
        isFor === "course" ? "448" : "330"
      }x200`}
      ...
      cardFooter={
        <>
          <Button
            startContent={
              <FontAwesomeIcon size="lg" className="mr-0.5" icon={faCartPlus} />
            }
            variant="flat"
            className="bg-purple-800 text-white text-sm hover:bg-purple-700 rounded-lg px-4"
          >
            Añadir al carrito
          </Button>
          <Button
            variant="light"
            color="secondary"
            className=" min-w-28 rounded-lg text-purple-300"
          >
            Ver detalles
          </Button>
        </>
    ...
    >
      <p className="text-gray-200 py-1 overflow-hidden text-ellipsis line-clamp-3">
        Conviértete en un experto en AppSheetHub y aprende a crear aplicaciones
